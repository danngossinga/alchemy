import { Credentials, CredentialsFromEnv, credentials, MachineIdentity, } from "@distilled.cloud/fly-io";
...
export const makeSecretAuth = (ambient) => ({
    authorize: (eff) => {
        if (globalThis.__ALCHEMY_RUNTIME__) {
            return eff.pipe(Effect.provide(Layer.mergeAll(CredentialsFromEnv, FetchHttpClient.layer)), Effect.timeout("8 seconds"));
        }
...
const flyMachineApiHttp = NodeHttpClient.layerNodeHttpNoAgent.pipe(Layer.provide(NodeHttpClient.layerAgentOptions({
...
export const makeKmsAuth = (ambient) => ({
    authorize: (eff) => {
        if (globalThis.__ALCHEMY_RUNTIME__) {
            return Effect.scoped(eff.pipe(Effect.provide(Layer.mergeAll(Layer.succeed(MachineIdentity, true), flyMachineApiHttp, credentials({
                apiKey: "unused",
