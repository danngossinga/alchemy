...
export const FunctionProvider = () => Provider.effect(Function, Effect.gen(function* () {
    ...
    const waitForFunctionUpdate = Effect.fn(function* (functionName, session) {
        return yield* Effect.gen(function* () {
            ...
            schedule: Schedule.spaced("2 seconds").pipe(Schedule.tap(({ attempt }) => session.note(`Waiting for Lambda image update before repository cleanup: ${functionName} (${attempt * 2}s)`))),
            times: 30,
        }));
    });
