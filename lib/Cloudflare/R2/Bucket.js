    ...
    return {
        ...
        reconcile: Effect.fn(function* ({ id, news = {}, output }) {
            ...
            if (news.lockRules !== undefined || lockRules !== undefined) {
                ...
                const observedRules = (current.rules ?? []).map((rule) => normalizeLockRule({ ...rule, prefix: rule.prefix ?? undefined }));
        ...
        read: Effect.fn(function* ({ id, output, olds }) {
            ...
            return yield* Effect.gen(function* () {
                ...
                const lockRules = output?.lockRules === undefined
                    ? undefined
                    : ((yield* r2.getBucketLock({
                        accountId: acct,
                        bucketName: name,
                        jurisdiction,
                    })).rules ?? []).map((rule) => normalizeLockRule({
                        ...rule,
