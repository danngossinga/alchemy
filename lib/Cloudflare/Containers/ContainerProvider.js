...
export const LiveContainerProvider = () => Provider.effect(ContainerPlatform, Effect.gen(function* () {
    ...
    const resolveRegistryDigest = Effect.fn("resolveRegistryDigest")(function* (imageRef, credentials) {
        ...
        const request = HttpClientRequest.head(manifestUrl).pipe(HttpClientRequest.basicAuth(credentials.username, credentials.password), HttpClientRequest.setHeader("Accept", "application/vnd.oci.image.manifest.v1+json, application/vnd.docker.distribution.manifest.v2+json"));
    ...
    const buildAndPushImage = Effect.fn("buildAndPushImage")(function* (id, props, build, imageRef, previousImageRef, session) {
    ...
    const upsertApplication = Effect.fn(function* ({ id, news, bindings, existing, durableObjects, session, }) {
        ...
        if (imageHash !== existing.hash?.image) {
            const published = yield* buildAndPushImage(id, news, build, imageRef, existing.hash?.digest === undefined
    ...
    return ContainerPlatform.Provider.of({
        ...
        precreate: Effect.fn(function* ({ id, news = {}, session }) {
            ...
            const { build, imageRef, imageHash, dev } = yield* computeImage(id, news, env);
            const published = yield* buildAndPushImage(id, news, build, imageRef, undefined, session);
