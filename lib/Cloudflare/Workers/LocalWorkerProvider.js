...
export const LocalWorkerProvider = () => LocalProvider.make(Worker, LOCAL_ENTRY_URL, Effect.gen(function* () {
    ...
    const serveBundleStream = Effect.fn(function* (worker, proxy, bundles) {
        ...
        yield* bundles.pipe(Stream.tap((event) => {
            if (event._tag === "Start") {
                ...
                return Effect.logError(`[${worker.fqn}] Bundle error`, event.error);
            }
            return Effect.void;
            ...
            if (exit._tag === "Success") {
                ...
                return Effect.logError(`[${worker.fqn}] Error`, Cause.squash(exit.cause));
            }
    ...
    Effect.uninterruptibleMask((restore) => Effect.gen(function* () {
        ...
        while (true) {
            ...
            yield* child.exitCode.pipe(Effect.flatMap((exitCode) => Effect.logWarning(`[${worker.fqn}] Dev server child exited unexpectedly with code ${exitCode}`)), Effect.andThen(proxy.unset().pipe(Effect.ignore)), Effect.andThen(invalidate), Effect.forkIn(scope));
