...
const fillEnvironmentId = (attrs) => {
    ...
    return railway.environments
        ...
        : attrs), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(attrs)));
};
...
const getById = (projectId) => railway.project({ id: projectId }).pipe(Effect.map((project) => (isGone(project) ? undefined : project)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
 ...
 */
export const projectEnvironmentIds = (project) => railway.environments.items({ projectId: project.projectId, first: 50 }).pipe(Stream.filter((env) => env.deletedAt == null), Stream.map((env) => env.id), Stream.runCollect, Effect.map((ids) => {
...
export const ProjectProvider = () => Provider.succeed(Project, {
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        yield* getById(projectId).pipe(Effect.map((project) => project === undefined), Effect.repeat({
