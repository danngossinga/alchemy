...
const resolveVariableProps = (props) => Effect.gen(function* () {
...
const VariableResource = Resource("Railway.Variable");
/**
 ...
 * @resource
 */
export const Variable = Object.assign((id, props) => VariableResource(id, resolveVariableProps(props)), VariableResource);
...
export class VariableEnvironmentRequired extends Data.TaggedError("Railway.VariableEnvironmentRequired") {
}
...
const digestOf = (plain) => Effect.sync(() => createHash("sha256").update(Buffer.from(plain, "utf8")).digest("hex"));
...
const getValue = (projectId, environmentId, name, serviceId) => listVariableMap(projectId, environmentId, serviceId).pipe(Effect.map((vars) => vars[name]));
...
export const VariableProvider = () => Provider.succeed(Variable, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(projects, (project) => Effect.gen(function* () {
            ...
            const nested = yield* Effect.forEach(envIds, (environmentId) => listVariableMap(project.projectId, environmentId).pipe(Effect.flatMap((vars) => Effect.forEach(Object.keys(vars).filter((name) => matchesAlchemyPhysicalName(name)), (name) => digestOf(vars[name]).pipe(Effect.map((digest) => toAttrs({
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        if (environmentId === undefined) {
            return yield* new VariableEnvironmentRequired({
                message: "Variable requires a Railway environment (pass environment or a Project with environmentId)",
            });
        }
        const serviceId = props.service !== undefined
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        yield* getValue(output.projectId, output.environmentId, output.name, output.serviceId).pipe(Effect.map((value) => value === undefined), Effect.repeat({
