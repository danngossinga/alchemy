...
import { factory as retryFactory } from "./RetryPolicy.js";
...
export const listServiceDomains = (projectId, environmentId, serviceId) => railway.domains({ environmentId, projectId, serviceId }).pipe(Effect.map((result) => result.serviceDomains.filter((domain) => !isGone(domain))), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed([])));
const findCloudDomainById = (input) => railway
    ...
    .pipe(Effect.map((result) => result.serviceDomains.find((candidate) => candidate.id === input.domainId)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
...
export const deleteOwnedServiceDomain = Effect.fn(function* (input) {
    ...
    for (const row of owned) {
        if (row.syncStatus === "DELETING")
            continue;
        yield* withEnvironmentConfigLock(input.environmentId, railway.deleteServiceDomain({ id: row.id })).pipe(Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.void), Effect.asVoid);
    }
    const gone = yield* listServiceDomains(input.projectId, input.environmentId, input.serviceId).pipe(Effect.map((rows) => !rows.some((row) => keys.has(row.id) ||
        (input.domain !== undefined && row.domain === input.domain))), Effect.repeat({
...
const domainCreateRetry = railway.Retry.policy((lastError) => {
    ...
    return {
        while: (error) => !(error instanceof railway.RailwayServiceDomainCreateFailed) &&
...
const createViaMutation = (input) => {
    ...
    const create = railway
        ...
        : Effect.succeed(created)), Effect.catchTag("RailwayServiceDomainCreateFailed", (error) => listedOrFail(error)), Effect.catchTag("RailwayNotFound", (error) => input.domainId !== undefined &&
        error.message.includes("ServiceInstance not found")
        ? listedOrFail(error)
        : Effect.fail(error)), Effect.catchTag("RailwayValidationError", (error) => input.domainId !== undefined || alreadyExists(error.message)
        ? listedOrFail(error)
        : Effect.fail(error)), Effect.catchTag("Conflict", () => listedOrFail(missing())));
