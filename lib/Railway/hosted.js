...
import { matchesPackageRoot, normalizeInstallTargets, parsePackageRootFromSpecifier, resolvePackageInstallIdentity, } from "../Bundle/InstalledPackages.js";
import { findCwdForBundle, getStableContextDir, resolveMainPath, } from "../Bundle/TempRoot.js";
import { safeHttpEffect } from "../Http.js";
 ...
 */
export const createRailwayFunctionRuntimeContext = (type) => (id) => {
    const base = createContainerRuntimeContext(type)(id);
    return {
        ...base,
        serve: ((handler, options) => Effect.sync(() => {
            if (!globalThis.__ALCHEMY_RUNTIME__)
                return;
            const run = safeHttpEffect(globalThis.__R?.(options?.shape, handler) ?? handler);
            globalThis.__aFF = async (request) => {
                try {
                    const response = await Effect.runPromise(run.pipe(Effect.provideService(HttpServerRequest.HttpServerRequest, HttpServerRequest.fromWeb(request)), Effect.scoped));
...
const dockerfileCopyLines = (files) => (files ?? []).flatMap((file) => {
 ...
 */
const makeNodeBootstrap = (handler) => (importPath) => `
 ...
 * this file.
 */
const makeFunctionBootstrap = (handler) => (importPath) => `
...
export class FunctionBundleNotSingleFile extends Data.TaggedError("Railway.FunctionBundleNotSingleFile") {
}
const decodeBundleText = (content) => typeof content === "string" ? content : new TextDecoder().decode(content);
...
const skipCanvasPin = (root) => CANVAS_PINNED.has(root) ||
    root === "alchemy" ||
    root.startsWith("@effect/platform-node") ||
    root.startsWith("@distilled.cloud/");
const collectCanvasPackageRoots = (source, install) => {
    const roots = new Set(Object.keys(install));
    for (const match of source.matchAll(IMPORT_SPEC)) {
        const spec = match[1] ?? match[2];
        if (spec === undefined)
            continue;
        const root = parsePackageRootFromSpecifier(spec);
        if (root !== undefined && !skipCanvasPin(root))
            roots.add(root);
    }
    return [...roots].sort();
};
const readPackageVersion = (name) => {
    try {
        ...
        const version = req(`${name}/package.json`)
            .version;
...
const pinImport = (name, version) => {
    const spec = version !== undefined && version.length > 0 && version !== "*"
        ? `${name}@${version}`
        : name;
 ...
 */
const wrapCanvasListener = (inner, install = {}) => Effect.sync(() => {
    ...
    const extraPins = collectCanvasPackageRoots(inner, install).map((name) => {
        ...
        const version = requested !== undefined && requested !== "*"
            ? requested
 ...
 */
const wrapAsyncCanvasListener = (inner, handler, install = {}) => Effect.sync(() => {
    const extraPins = collectCanvasPackageRoots(inner, install).map((name) => {
        const requested = install[name];
        const version = requested !== undefined && requested !== "*"
            ? requested
...
const isCanvasFunctionExternal = (moduleId) => {
    if (moduleId.startsWith("@effect/platform-node"))
        return false;
    if (moduleId === "node:os")
        return false;
    return (moduleId === "effect" ||
...
const createBundleProgram = (virtualEntryPlugin, options) => Effect.fn(function* (props) {
    ...
    const bundleOutput = yield* buildBundle(realMain, options?.skipVirtualEntry === true
        ? undefined
 ...
 */
export const createRailwayFunctionSupport = ({ stackName, stage, virtualEntryPlugin, }) => {
    const alchemyEnv = {
    ...
    const bundleToSource = Effect.fn(function* (props) {
        const bundled = props.isExternal === true
        ...
        const source = props.isExternal === true
...
export const createRailwayHostedSupport = ({ stackName, stage, virtualEntryPlugin, dotAlchemy, }) => {
    const alchemyEnv = {
        ALCHEMY_STACK_NAME: stackName,
        ALCHEMY_STAGE: stage,
        ALCHEMY_PHASE: "runtime",
