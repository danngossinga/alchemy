...
import { UrlMissing as RedisUrlMissing } from "../Redis/index.js";
...
const resolveName = (redis) => Effect.gen(function* () {
    ...
    if (!globalThis.__ALCHEMY_RUNTIME__) {
        ...
        if (isRailwayHost(host)) {
            ...
            yield* host.bind `${redis}`({
                env: {
                    [REDIS_URL_ENV]: Output.map(Output.asOutput(redis.name), (name) => `\${{${name}.${REDIS_URL_ENV}}}`),
                },
            });
        }
    }
