...
import { ensureServiceDomain, } from "./ServiceDomain.js";
import { collectBindingState, createRailwayFunctionSupport, createRailwayFunctionRuntimeContext, DEFAULT_PORT, plainEnvValue, toEnvRecord, } from "./hosted.js";
import { mintRpcToken, RPC_TOKEN_ENV } from "./rpc-token.js";
export { FunctionBundleNotSingleFile } from "./hosted.js";
/** Canvas Functions run on the Bun function runtime. */
export const FUNCTION_RUNTIME_NAME = "bun";
 ...
 */
export const isFunctionImage = (image) => image != null && image.startsWith("ghcr.io/railwayapp/function");
export const isFunction = (value) => typeof value === "object" &&
    value !== null &&
    value.Type === "Railway.Function";
const resolveFunctionProps = (id, props) => Effect.gen(function* () {
 ...
 * @resource
 */
export const Function = Platform("Railway.Function", {
...
export class FunctionRuntimeImageMissing extends Data.TaggedError("Railway.FunctionRuntimeImageMissing") {
}
...
const wantsHttp = (props) => props.http === true ||
    (props.http !== false && props.cronSchedule === undefined);
const hashSource = (source) => Effect.sync(() => createHash("sha256").update(source).digest("hex"));
const startCommandOf = (source) => Effect.gen(function* () {
...
const resolveSource = (props) => Effect.gen(function* () {
    ...
    return yield* new FunctionSourceRequired({
        message: "Railway.Function requires `source` (inline TypeScript), `path` to a single file, or `main` (Effect-native).",
    });
});
const latestRuntimeImage = () => railway.functionRuntime({ name: FUNCTION_RUNTIME_NAME }).pipe(Effect.flatMap((runtime) => {
...
const hostedProgramProps = (props, port) => {
    if (props.main === undefined || props.main.length === 0)
        return undefined;
    return {
        main: props.main,
...
export const FunctionProvider = () => Provider.effect(Function, Effect.gen(function* () {
    ...
    return Function.Provider.of({
        ...
        diff: Effect.fn(function* ({ news, output }) {
            ...
            const hasSource = (news.source !== undefined && news.source.length > 0) ||
            ...
            const rows = yield* Effect.forEach(projects, (project) => Effect.gen(function* () {
                ...
                const items = yield* Effect.forEach(services.filter((service) => matchesAlchemyPhysicalName(service.name)), (service) => Effect.gen(function* () {
                    for (const environmentId of envIds) {
                        const instance = yield* getInstance(environmentId, service.id);
                        const image = instance?.source?.image ?? undefined;
                        if (!isFunctionImage(image))
                            continue;
                        return toAttrs({
                            service,
                            instance,
        ...
        reconcile: Effect.fn(function* ({ id, news, output, bindings }) {
            ...
            if (environmentId === undefined) {
                return yield* new FunctionProjectRequired({
                    message: "Function requires a Railway environment (pass environment or a Project with environmentId)",
                });
            }
            ...
            const program = hostedProgramProps(props, props.port ?? (props.main !== undefined ? DEFAULT_PORT : undefined));
            let source;
            let codeHash;
            if (program !== undefined) {
