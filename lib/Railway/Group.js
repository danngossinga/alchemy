...
const getProject = (projectId) => railway.project({ id: projectId }).pipe(Effect.map((project) => (project.deletedAt != null ? undefined : project)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const getEnvironment = (environmentId, projectId) => railway
...
const commitPatch = (input) => withEnvironmentConfigLock(input.environmentId, railway.environmentPatchCommit({
    ...
    patch: input.patch,
})).pipe(Effect.catchTag(["RailwayValidationError", "RailwayInternalError"], () => Effect.succeed("")));
...
const waitUntilGone = (input) => observe({
    projectId: input.projectId,
    environmentId: input.environmentId,
    groupId: input.groupId,
    name: input.name,
}).pipe(Effect.map((group) => group === undefined), Effect.repeat({
    schedule: Schedule.spaced("1 second"),
    until: (gone) => gone,
    times: 4,
}));
...
export const GroupProvider = () => Provider.succeed(Group, {
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(projects, (project) => railway.project({ id: project.projectId }).pipe(Effect.map((live) => live.groups.edges.flatMap((edge) => {
