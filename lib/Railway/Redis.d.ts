...
export declare const DEFAULT_REDIS_IMAGE = "redis:7";
export declare const REDIS_PORT = 6379;
import { REDIS_URL_ENV } from "./RedisBinding.ts";
export { REDIS_URL_ENV };
export declare const REDIS_PASSWORD_ENV = "REDISPASSWORD";
...
export type Redis = Resource<"Railway.Redis", RedisProps, {
    /** Railway service id. */
    serviceId: string;
...
declare const RedisResource: import("../Resource.ts").ResourceClass<Redis>;
/**
 ...
 * @resource
 */
export declare const Redis: typeof RedisResource;
...
declare const RedisNotCreated_base: new <A extends Record<string, any> = {}>(args: import("effect/Types").VoidIfEmpty<{ readonly [P in keyof A as P extends "_tag" ? never : P]: A[P]; }>) => import("effect/Cause").YieldableError & {
    readonly _tag: "Railway.RedisNotCreated";
} & Readonly<A>;
...
declare const RedisProjectRequired_base: new <A extends Record<string, any> = {}>(args: import("effect/Types").VoidIfEmpty<{ readonly [P in keyof A as P extends "_tag" ? never : P]: A[P]; }>) => import("effect/Cause").YieldableError & {
    readonly _tag: "Railway.RedisProjectRequired";
} & Readonly<A>;
export declare class RedisProjectRequired extends RedisProjectRequired_base<{
    message: string;
}> {
}
declare const RedisDeployFailed_base: new <A extends Record<string, any> = {}>(args: import("effect/Types").VoidIfEmpty<{ readonly [P in keyof A as P extends "_tag" ? never : P]: A[P]; }>) => import("effect/Cause").YieldableError & {
    readonly _tag: "Railway.RedisDeployFailed";
} & Readonly<A>;
export declare class RedisDeployFailed extends RedisDeployFailed_base<{
    serviceId: string;
    status: string;
    deploymentId: string | undefined;
}> {
}
export declare const isRedisImage: (image: string | null | undefined) => boolean;
export { command as redisCommand, connectionUrl as redisConnectionUrl, run as runRedisCommand, } from "../Redis/index.ts";
export declare const RedisProvider: () => import("effect/Layer").Layer<Provider.Provider<Redis>, never, import("./Environment.ts").RailwayEnvironment | import("../Stack.ts").Stack | import("../Stage.ts").Stage | railway.RailwayOpContext>;
