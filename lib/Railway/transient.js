 ...
 */
export const isRailwayTransient = (e) => e._tag === "RailwayRateLimited" ||
...
const jitterUpTo = (max) => Duration.millis(Math.random() * Duration.toMillis(max));
/**
 * 30s plus 0–30s jitter. Railway's public API is ~10k requests/hour;
 * concurrent suite retries have to stay sparse.
 */
export const conservativeSpacing = Schedule.spaced("30 seconds").pipe(Schedule.addDelay(() => Effect.sync(() => jitterUpTo(Duration.seconds(30)))));
/**
...
const retryCreateRateLimit = (effect) => effect.pipe(Effect.catch((error) => error instanceof railway.RailwayRateLimited
    ? Effect.sleep(Duration.sum(error.retryAfter ?? createRateLimitWait, jitterUpTo(Duration.seconds(30)))).pipe(Effect.andThen(retryCreateRateLimit(effect)))
    : Effect.fail(error)));
/**
 ...
 */
export const waitOutCreateRateLimit = (effect) => Semaphore.withPermits(projectAndEnvironmentCreateGate, 1)(retryCreateRateLimit(effect));
/**
