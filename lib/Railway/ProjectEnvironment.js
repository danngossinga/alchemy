...
const getById = (environmentId, projectId) => railway
    ...
    .pipe(Effect.map((env) => (isGone(env) ? undefined : env)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const findByName = (projectId, name) => railway.environments.items({ projectId, first: 50 }).pipe(Stream.filter((env) => !isGone(env) && env.name === name), Stream.take(1), Stream.runHead, Effect.map((option) => (option._tag === "Some" ? option.value : undefined)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const listProjectEnvironments = (projectId) => railway.environments.items({ projectId, first: 50 }).pipe(Stream.runCollect, Effect.map((envs) => Array.from(envs)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed([])));
export const EnvironmentProvider = () => Provider.succeed(Environment, {
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        yield* getById(environmentId, output.projectId).pipe(Effect.map((env) => env === undefined), Effect.repeat({
