...
const resolveSandboxProps = (props) => Effect.gen(function* () {
...
const SandboxResource = Resource("Railway.Sandbox");
/**
 ...
 * @resource
 */
export const Sandbox = Object.assign((id, props) => SandboxResource(id, resolveSandboxProps(props)), SandboxResource);
export class SandboxNotCreated extends Data.TaggedError("Railway.SandboxNotCreated") {
}
export class SandboxEnvironmentRequired extends Data.TaggedError("Railway.SandboxEnvironmentRequired") {
}
export class SandboxFailed extends Data.TaggedError("Railway.SandboxFailed") {
}
export class SandboxCheckpointNotFound extends Data.TaggedError("Railway.SandboxCheckpointNotFound") {
}
...
const isGone = (sandbox) => sandbox === undefined ||
    sandbox.status === "DESTROYED" ||
    sandbox.status === "DESTROYING";
const toAttrs = (sandbox, fallback) => ({
...
const toTemplateInput = (template) => ({
...
const getById = (environmentId, sandboxId) => railway.sandbox({ environmentId, id: sandboxId }).pipe(Effect.map((sandbox) => (isGone(sandbox) ? undefined : sandbox)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const listSandboxes = (environmentId) => railway.sandboxes.items({ environmentId, first: 50 }).pipe(Stream.filter((sandbox) => !isGone(sandbox)), Stream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag([
    "RailwayNotFound",
    "NotFound",
    "RailwayForbidden",
    "Forbidden",
...
const waitUntilRunning = (environmentId, sandboxId) => Effect.gen(function* () {
...
}), Effect.catchTag("Railway.SandboxPending", () => getById(environmentId, sandboxId)));
const waitUntilGone = (environmentId, sandboxId) => getById(environmentId, sandboxId).pipe(Effect.map((sandbox) => sandbox === undefined), Effect.repeat({
...
const findCheckpoint = (items, name) => items.find((item) => item.key === name);
/**
...
export const Exec = Binding.Service("Railway.Sandbox.Exec");
/**
 ...
 */
export const ExecHttp = Layer.effect(Exec, Effect.succeed(Effect.fn(function* (sandbox) {
    const sandboxId = sandbox.sandboxId;
    const environmentId = sandbox.environmentId;
    return ((request) => execSandbox({
        sandboxId,
        environmentId,
...
export const SandboxProvider = () => Provider.succeed(Sandbox, {
    ...
    diff: Effect.fn(function* ({ news, olds, output }) {
        ...
        const idleChanged = news.idleTimeoutMinutes !== undefined &&
            news.idleTimeoutMinutes !== output.idleTimeoutMinutes;
        const isolationChanged = news.networkIsolation !== undefined &&
            news.networkIsolation !== output.networkIsolation;
        const templateChanged = news.template !== undefined &&
            templateKey(news.template) !== templateKey(olds?.template);
        const variablesChanged = news.variables !== undefined &&
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(projects, (project) => Effect.gen(function* () {
            ...
            const nested = yield* Effect.forEach(envIds, (environmentId) => listSandboxes(environmentId).pipe(Effect.map((items) => items.map((item) => toAttrs(item, { projectId: project.projectId })))));
            return nested.flat();
        }));
        const seen = new Set();
        const unique = [];
        for (const row of rows.flat()) {
            if (seen.has(row.sandboxId))
                continue;
    ...
    reconcile: Effect.fn(function* ({ news, output }) {
        ...
        if (environmentId === undefined) {
            return yield* new SandboxEnvironmentRequired({
                message: "Sandbox requires a Railway environment (pass a Project, Environment, or { environmentId })",
            });
        }
        const projectId = projectIdOf(props.environment) ?? output?.projectId;
        let current = output?.sandboxId !== undefined && output.sandboxId.length > 0
    ...
    delete: Effect.fn(function* ({ output }) {
        const sandboxId = output.sandboxId;
        const environmentId = output.environmentId;
        if (sandboxId.length === 0 || environmentId.length === 0)
            return;
        yield* railway
