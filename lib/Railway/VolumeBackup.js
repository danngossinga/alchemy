...
const VolumeBackupResource = Resource("Railway.VolumeBackup");
/**
 ...
 * @resource
 */
export const VolumeBackup = Object.assign((id, props) => VolumeBackupResource(id, resolveVolumeBackupProps(props)), VolumeBackupResource);
export class VolumeBackupNotCreated extends Data.TaggedError("Railway.VolumeBackupNotCreated") {
}
export class VolumeBackupVolumeRequired extends Data.TaggedError("Railway.VolumeBackupVolumeRequired") {
}
export class VolumeBackupWorkflowFailed extends Data.TaggedError("Railway.VolumeBackupWorkflowFailed") {
}
class VolumeBackupPending extends Data.TaggedError("Railway.VolumeBackupPending") {
}
const volumeInstanceIdOf = (value) => {
...
const transientState = (state) => state === "UPDATING" ||
    ...
    state === "RESTORING";
const isGoneInstance = (instance) => instance === undefined ||
    instance.deletedAt != null ||
    instance.isPendingDeletion ||
    goneState(instance.state);
const uniqueKinds = (kinds) => {
    const seen = new Set();
    const out = [];
...
const kindsKey = (kinds) => [...kinds].slice().sort().join(",");
const resolveName = (id, existing) => Effect.gen(function* () {
    if (existing !== undefined && existing.length > 0)
        return existing;
    return yield* createRailwayName(id);
});
const listBackups = (volumeInstanceId) => railway
    .listVolumeInstanceBackup({ volumeInstanceId })
    .pipe(Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed([])));
const listSchedules = (volumeInstanceId) => railway.listVolumeInstanceBackupSchedule({ volumeInstanceId }).pipe(Effect.map((items) => uniqueKinds(items.map((item) => item.kind))), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed([])));
const toAttrs = (backup, context) => ({
...
const findBackup = (backups, match) => {
...
const getByInstanceId = (volumeInstanceId) => railway.volumeInstance({ id: volumeInstanceId }).pipe(Effect.map((instance) => (isGoneInstance(instance) ? undefined : instance)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const listVolumeInstances = (environmentId, projectId) => railway.environment({ id: environmentId, projectId }).pipe(Effect.map((env) => env.deletedAt != null
    ...
    : env.volumeInstances.edges
        .map((edge) => edge.node)
        .filter((node) => !isGoneInstance(node))), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed([])));
...
const waitForWorkflow = (workflowId, volumeInstanceId, mode) => Effect.gen(function* () {
    const result = yield* railway.workflowStatus({ workflowId }).pipe(Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed({
        status: "NotFound",
        error: null,
    })));
...
}), Effect.catchTag("Railway.VolumeBackupPending", () => mode === "delete"
    ? Effect.void
    : Effect.fail(new VolumeBackupWorkflowFailed({
        workflowId,
        error: "timed out waiting for backup workflow",
    }))));
const instanceReady = (instance) => instance !== undefined &&
    !isGoneInstance(instance) &&
    !transientState(instance.state) &&
    instance.state !== "ERROR";
const waitUntilReady = (volumeInstanceId) => getByInstanceId(volumeInstanceId).pipe(Effect.flatMap((instance) => {
    if (!instanceReady(instance)) {
        return Effect.fail(new VolumeBackupPending({
            volumeInstanceId,
...
const waitForBackup = (input) => listBackups(input.volumeInstanceId).pipe(Effect.flatMap((backups) => {
    const found = findBackup(backups, { id: input.id, name: input.name }) ??
    ...
    times: 10,
    schedule: Schedule.spaced("2 seconds"),
}), Effect.catchTag("Railway.VolumeBackupPending", () => listBackups(input.volumeInstanceId).pipe(Effect.map((backups) => findBackup(backups, { id: input.id, name: input.name }) ??
    backups.find((backup) => !input.previousIds.has(backup.id))))));
const waitUntilGone = (volumeInstanceId, volumeInstanceBackupId) => listBackups(volumeInstanceId).pipe(Effect.map((backups) => !backups.some((backup) => backup.id === volumeInstanceBackupId)), Effect.repeat({
    schedule: Schedule.spaced("1 second"),
    until: (gone) => gone,
    times: 8,
}));
const observeInstance = Effect.fn(function* (input) {
    if (input.volumeInstanceId !== undefined &&
...
export const VolumeBackupProvider = () => Provider.succeed(VolumeBackup, {
    stables: [
    ...
    diff: Effect.fn(function* ({ news, output }) {
        ...
        const instanceChanged = nextInstance !== undefined && nextInstance !== output.volumeInstanceId;
        const nextEnv = environmentIdOf(news.environment);
        const environmentChanged = nextEnv !== undefined &&
            output.environmentId !== undefined &&
            nextEnv !== output.environmentId;
        const nextVolumeId = volumeIdOf(news.volume);
        const volumeChanged = nextVolumeId !== undefined &&
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(projects, (project) => Effect.gen(function* () {
            const envRows = yield* railway.environments
                ...
                .pipe(Stream.filter((env) => env.deletedAt == null), Stream.runCollect, Effect.map((chunk) => Array.from(chunk)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed([])));
            const instances = envRows.flatMap((env) => env.volumeInstances.edges
                .map((edge) => edge.node)
                .filter((instance) => instance.deletedAt == null &&
                matchesAlchemyPhysicalName(instance.volume.name)));
            const nested = yield* Effect.forEach(instances, (instance) => Effect.gen(function* () {
                const backups = yield* listBackups(instance.id);
                const schedules = yield* listSchedules(instance.id);
                return backups.map((backup) => toAttrs(backup, {
        ...
        for (const row of rows.flat()) {
            if (seen.has(row.volumeInstanceBackupId))
                continue;
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        if (props.lock === true && current.expiresAt != null) {
            ...
            const locked = yield* listBackups(volumeInstanceId).pipe(Effect.map((backups) => findBackup(backups, { id: current.id, name })));
            if (locked !== undefined)
                current = locked;
        }
    ...
    delete: Effect.fn(function* ({ output }) {
        ...
        const volumeInstanceId = output.volumeInstanceId;
        if (volumeInstanceBackupId.length === 0 ||
        ...
        const deleted = yield* railway
            ...
            .pipe(Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed({ workflowId: null })));
