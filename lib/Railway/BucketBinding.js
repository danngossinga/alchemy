...
const scopeFromResource = (bucket) => Effect.gen(function* () {
    const bucketName = (yield* readValue(bucket.s3BucketName)) ??
    ...
    const region = (yield* readValue(bucket.s3Region)) ??
...
const scopeFromEnv = Effect.gen(function* () {
    const bucketName = yield* Config.string("BUCKET_NAME").pipe(Config.orElse(() => Config.string("AWS_S3_BUCKET_NAME")));
...
export const makeRailwayS3Binding = (options) => Effect.succeed(Effect.fn(function* (bucket) {
