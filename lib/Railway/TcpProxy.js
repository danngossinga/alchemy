...
const listProxies = (environmentId, serviceId) => railway.tcpProxies({ environmentId, serviceId }).pipe(Effect.map((items) => items.filter((proxy) => !isGone(proxy))), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed([])));
...
const waitUntilGone = (environmentId, serviceId, id) => findById(environmentId, serviceId, id).pipe(Effect.map((proxy) => proxy === undefined), Effect.repeat({
