...
const isThrottled = (error) => error instanceof railway.TooManyRequests ||
 ...
 *   15s per delay.
 */
export const factory = (lastError) => {
    const base = railway.Retry.makeDefault(lastError);
    return {
        while: base.while,
        schedule: Schedule.max([
            Schedule.exponential(500, 2).pipe(railway.Retry.capped(Duration.seconds(15)), Schedule.modifyDelay(({ duration }) => Effect.gen(function* () {
...
export const RailwayRetryPolicy = Layer.succeed(railway.Retry.Retry, factory);
