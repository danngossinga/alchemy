...
import { assertHostDisk } from "./MountVolume.js";
import { ownedProjects } from "./Project.js";
import { listServiceVolumes } from "./Volume.js";
import { deleteOwnedServiceDomain, ensureServiceDomain, findServiceDomainById, } from "./ServiceDomain.js";
import { collectBindingState, createRailwayHostedSupport, DEFAULT_PORT, plainEnvValue, toEnvRecord, } from "./hosted.js";
import { RPC_TOKEN_ENV } from "./rpc-token.js";
import { tarGzipDirectory } from "../Util/tarGzip.js";
import { hashRailwayLocalContext, prepareRailwayLocalContext, resolveRailwayServiceSource, ServiceSourceInvalid, } from "./local-context.js";
import { uploadDeployTarball } from "./Up.js";
import { Service } from "./Service.js";
export { ServiceContextPathInvalid, ServiceContextPathUnsupported, ServiceContextSymlinkUnsupported, ServiceContextTooLarge, ServiceDockerfileOutsideContext, ServiceDockerfilePathInvalid, ServiceImageOrMainRequired, ServiceSourceInvalid, } from "./local-context.js";
...
const getById = (serviceId) => railway.service({ id: serviceId }).pipe(Effect.map((service) => (isGoneService(service) ? undefined : service)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const getInstance = (environmentId, serviceId) => railway.serviceInstance({ environmentId, serviceId }).pipe(Effect.map((instance) => (isGoneInstance(instance) ? undefined : instance)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const listProjectServices = (projectId) => railway.project({ id: projectId }).pipe(Effect.map((project) => project.services.edges
    .map((edge) => edge.node)
    .filter((node) => !isGoneService(node))), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed([])));
...
const alreadyExists = (message) => /already exists|already in use|duplicate/i.test(message);
...
const deployReady = (status) => status === "SUCCESS" || status === "SLEEPING";
const deployFailed = (status) => status === "FAILED" || status === "CRASHED" || status === "REMOVED";
const undef = (value) => value == null ? undefined : value;
const sameWatchPatterns = (observed, desired) => desired === undefined || deepEqual([...(observed ?? [])], [...desired]);
const samePreDeployCommand = (observed, desired) => desired === undefined ||
    (desired === null
        ...
        : observed === desired ||
            (Array.isArray(observed) &&
                observed.length === 1 &&
                observed[0] === desired));
const assignIfChanged = (input, key, desired, observed) => {
    if (desired === undefined)
        return false;
    if (desired === null
        ? observed == null
        : deepEqual(undef(observed), desired)) {
        return false;
    }
    input[key] = desired;
    return true;
...
const instanceSettingsDelta = (input) => {
    ...
    else if (input.sourceImage !== undefined &&
    ...
    changed =
        assignIfChanged(delta, "region", input.props.region, instance?.region) ||
            changed;
    changed =
        assignIfChanged(delta, "rootDirectory", input.props.rootDirectory, instance?.rootDirectory) || changed;
    changed =
        assignIfChanged(delta, "buildCommand", input.props.buildCommand, instance?.buildCommand) || changed;
    const preDeployCommand = input.props.preDeploy === undefined
        ? undefined
    ...
    changed =
        assignIfChanged(delta, "startCommand", input.props.startCommand, instance?.startCommand) || changed;
    changed =
        assignIfChanged(delta, "healthcheckPath", input.props.healthcheckPath ?? input.props.healthcheck, instance?.healthcheckPath) || changed;
    changed =
        assignIfChanged(delta, "healthcheckTimeout", input.props.healthcheckTimeout, instance?.healthcheckTimeout) || changed;
    changed =
        assignIfChanged(delta, "cronSchedule", input.props.cronSchedule, instance?.cronSchedule) || changed;
    changed =
        assignIfChanged(delta, "restartPolicyType", input.props.restartPolicyType, instance?.restartPolicyType) || changed;
    changed =
        assignIfChanged(delta, "restartPolicyMaxRetries", input.props.restartPolicyMaxRetries, instance?.restartPolicyMaxRetries) || changed;
    changed =
        assignIfChanged(delta, "drainingSeconds", input.props.drainingSeconds, instance?.drainingSeconds) || changed;
    changed =
        assignIfChanged(delta, "overlapSeconds", input.props.overlapSeconds, instance?.overlapSeconds) || changed;
    changed =
        assignIfChanged(delta, "sleepApplication", input.props.sleepApplication, instance?.sleepApplication) || changed;
    changed =
        assignIfChanged(delta, "dockerfilePath", input.props.dockerfilePath, instance?.dockerfilePath) || changed;
    changed =
        assignIfChanged(delta, "builder", input.props.builder, instance?.builder) ||
            changed;
    const watchPatterns = input.props.watchPatterns;
    if (watchPatterns !== undefined &&
...
const listDeploymentTriggers = (projectId, environmentId, serviceId) => railway.deploymentTriggers
    .items({
    projectId,
    environmentId,
    serviceId,
    ...
    .pipe(Stream.runCollect, Effect.map((triggers) => Array.from(triggers)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed([])));
const syncBranch = Effect.fn(function* (input) {
    if (input.branch === undefined)
        return false;
    const triggers = yield* listDeploymentTriggers(input.projectId, input.environmentId, input.serviceId);
...
const fetchDeployLogs = (deploymentId) => deploymentId === undefined || deploymentId.length === 0
    ? Effect.succeed("")
    : railway.deploymentLogs({ deploymentId, limit: 80 }).pipe(Effect.map((rows) => rows
        .map((row) => row.severity != null
        ? `[${row.severity}] ${row.message}`
        : row.message)
        .join("\n")), Effect.orElseSucceed(() => ""));
const waitForDeployment = (environmentId, serviceId) => Effect.gen(function* () {
...
const waitForDeploymentById = (input) => Effect.gen(function* () {
    ...
    const latest = instance?.latestDeployment;
    const match = latest?.id === input.deploymentId
        ? latest
        : (instance?.activeDeployments ?? []).find((deployment) => deployment.id === input.deploymentId);
...
const listVariableMap = (projectId, environmentId, serviceId) => railway
    ...
    .pipe(Effect.map(asVariableMap), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed({})));
const syncEnv = Effect.fn(function* (input) {
    if (Object.keys(input.desired).length === 0)
        return false;
...
export const ServiceProvider = () => Provider.effect(Service, Effect.gen(function* () {
    ...
    return Service.Provider.of({
        ...
        read: Effect.fn(function* ({ id, olds, output }) {
            ...
            const domain = output?.domainId !== undefined &&
        ...
        list: Effect.fn(function* () {
            ...
            const rows = yield* Effect.forEach(projects, (project) => listProjectServices(project.projectId).pipe(Effect.map((services) => services
        ...
        precreate: Effect.fn(function* ({ id, news }) {
            const name = yield* resolveName(id, typeof news.name === "string" ? news.name : undefined);
        ...
        reconcile: Effect.fn(function* ({ id, news, olds, output, bindings, session, }) {
            ...
            if (environmentId === undefined) {
                return yield* new ServiceProjectRequired({
                    message: "Service requires a Railway environment (pass environment or a Project with environmentId)",
                });
            }
            ...
            const localContext = source.mode === "context" ? source : undefined;
            const bound = collectBindingState(bindings ?? []);
            yield* assertHostDisk({
                name,
                mounts: bound.mounts,
            });
            const port = hostedMain !== undefined
            ...
            let hashed;
            if (source.mode === "main") {
                yield* (session?.note ?? ((_message) => Effect.void))(`Bundling ${id} program...`);
            ...
            if (current === undefined) {
                const created = yield* railway
                    ...
                    .pipe(Effect.catchTag("RailwayValidationError", (e) => alreadyExists(e.message)
                    ? Effect.succeed(undefined)
                    : Effect.fail(e)), Effect.catchTag("Conflict", () => Effect.succeed(undefined)));
            ...
            if (publicDomain) {
                ...
                yield* (session?.note ?? ((_message) => Effect.void))(`Creating service domain for ${id}...`);
            ...
            let localSourceChanged = localContext !== undefined &&
            ...
            const clearsDockerfilePath = leavingContext &&
                (source.mode === "image" ||
                    (source.mode === "repo" && props.dockerfilePath === undefined));
            const dockerfilePath = localPrepared?.dockerfilePath ??
            ...
            const shouldUpload = uploadSource &&
                (codeHash !== output?.code.hash || !latestOk || localSourceChanged);
            if (shouldUpload) {
                const note = session?.note ?? ((_message) => Effect.void);
                yield* note(`Preparing ${id} build context...`);
                const tarball = localPrepared !== undefined
        ...
        delete: Effect.fn(function* ({ output }) {
            ...
            yield* railway
                ...
                .pipe(Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.void));
            yield* getById(serviceId).pipe(Effect.map((service) => service === undefined), Effect.repeat({
