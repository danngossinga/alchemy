 ...
 */
const anonymousRailwayCredentials = (apiBaseUrl) => railway.CredentialsFromToken({
    token: "",
    tokenKind: "account",
    apiBaseUrl,
});
 ...
 */
const missingSession = ["RailwayNotFound", "NotFound"];
export const pollLoginSessionToken = (code) => railway.verifyLoginSession({ code }).pipe(Effect.catchTag(missingSession, () => Effect.succeed(false)), Effect.flatMap(() => railway
    .loginSessionConsume({ code })
    .pipe(Effect.catchTag(missingSession, () => Effect.succeed(null)))), Effect.map((token) => token != null && token.length > 0 ? token : undefined), Effect.repeat({
