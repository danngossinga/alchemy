...
import { createProject, environmentIdOf as projectEnvironmentId, ownedProjects, workspaceIdOf as projectWorkspaceId, } from "./Project.js";
const resolveTemplateProps = (props) => Effect.gen(function* () {
...
const TemplateResource = Resource("Railway.Template");
/**
 ...
 * @resource
 */
export const Template = Object.assign((id, props) => TemplateResource(id, resolveTemplateProps(props)), TemplateResource);
...
const isGoneProject = (project) => project === undefined || project.deletedAt != null;
...
const environmentIdFromProject = (project) => projectEnvironmentId(project);
const toAttrs = (input) => ({
...
const getProject = (projectId) => railway.project({ id: projectId }).pipe(Effect.map((project) => (isGoneProject(project) ? undefined : project)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const getTemplateById = (id) => railway
    .template({ id })
    .pipe(Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const getTemplateByCode = (code) => railway
    .template({ code })
    .pipe(Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const findPublished = (needle) => railway.templates.items({ first: 50 }).pipe(Stream.filter((template) => template.id === needle || template.code === needle), Stream.take(1), Stream.runHead, Effect.map((option) => (option._tag === "Some" ? option.value : undefined)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const resolveMarketplaceTemplate = (templateId) => Effect.gen(function* () {
...
const sourceForProject = (projectId) => railway
    .templateSourceForProject({ projectId })
    .pipe(Effect.catchTag(["RailwayNotFound", "NotFound", "RailwayForbidden"], () => Effect.succeed(undefined)));
...
const hydrateService = (serviceId) => railway.service({ id: serviceId }).pipe(Effect.map((service) => (isGoneService(service) ? undefined : service)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const hydrateServices = (services) => Effect.forEach(services, (service) => hydrateService(service.id), {
    concurrency: 4,
}).pipe(Effect.map((rows) => rows.filter((row) => row !== undefined)));
...
const parseConfig = (config) => Effect.sync(() => {
...
const waitForWorkflow = (workflowId, projectId) => Effect.gen(function* () {
...
const waitForServices = (input) => listProjectServices(input.projectId).pipe(Effect.flatMap((listed) => hydrateServices(listed)), Effect.flatMap((services) => {
    ...
    if (matched.length === 0) {
        return Effect.fail(new TemplatePending({
...
}), Effect.catchTag("Railway.TemplatePending", () => listProjectServices(input.projectId).pipe(Effect.flatMap((listed) => hydrateServices(listed)), Effect.map((services) => matchingServices({
    services,
    templateId: input.templateId,
    sourceId: input.sourceId,
    ownsProject: input.ownsProject,
})))));
const waitUntilServiceGone = (serviceId) => hydrateService(serviceId).pipe(Effect.map((service) => service === undefined), Effect.repeat({
    schedule: Schedule.spaced("1 second"),
    until: (gone) => gone,
    times: 8,
}));
const waitUntilProjectGone = (projectId) => getProject(projectId).pipe(Effect.map((project) => project === undefined), Effect.repeat({
    schedule: Schedule.spaced("1 second"),
    until: (gone) => gone,
    times: 8,
}));
...
export const TemplateProvider = () => Provider.succeed(Template, {
    ...
    diff: Effect.fn(function* ({ news, output }) {
        ...
        if (output === undefined)
            return undefined;
        const templateChanged = news.templateId !== output.templateId &&
    ...
    list: Effect.fn(function* () {
        ...
        const rows = yield* Effect.forEach(projects, (project) => Effect.gen(function* () {
            ...
            const stampedId = services.find((service) => service.templateId != null)?.templateId;
    ...
    reconcile: Effect.fn(function* ({ id, news, output }) {
        ...
        let environmentId = environmentIdOf(props.environment) ??
            environmentIdOf(props.project) ??
            output?.environmentId;
        let workspaceId = workspaceIdOf(props.project) ??
            output?.workspaceId ??
            (yield* currentWorkspaceId());
        let current = projectId !== undefined && projectId.length > 0
        ...
        if (services.length === 0) {
            ...
            if (deployed.projectId.length > 0 &&
