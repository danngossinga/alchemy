...
export class RailwayEnvironment extends Context.Service()("Railway::Environment") {
...
export const resolveWorkspace = Effect.fn(function* () {
    const fromMe = yield* railway.me({}).pipe(Effect.map((me) => me.workspace ?? me.workspaces[0]), Effect.catchTag(["RailwayForbidden", "RailwayUnauthenticated"], () => Effect.succeed(undefined)));
    ...
    if (workspace === undefined || workspace.id.length === 0) {
        return yield* new RailwayWorkspaceNotFound({
            message: "Railway current token did not include a workspace. Check RAILWAY_API_TOKEN is an account or workspace token.",
        });
    }
    return {
        id: workspace.id,
 ...
 */
export const fromCredentials = () => Layer.effect(RailwayEnvironment, Effect.gen(function* () {
    const creds = yield* Credentials;
    const http = yield* HttpClient.HttpClient;
    return yield* creds.pipe(Effect.flatMap((resolved) => resolveWorkspaceId().pipe(Effect.provide(Layer.mergeAll(Layer.succeed(Credentials, creds), Layer.succeed(HttpClient.HttpClient, http))), Effect.map((workspaceId) => ({
        ...resolved,
        workspaceId,
