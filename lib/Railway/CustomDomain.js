...
const getById = (customDomainId, projectId) => railway.customDomain({ id: customDomainId, projectId }).pipe(Effect.map((domain) => (isGone(domain) ? undefined : domain)), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed(undefined)));
const listServiceDomains = (projectId, environmentId, serviceId) => railway.domains({ environmentId, projectId, serviceId }).pipe(Effect.map((result) => result.customDomains.filter((domain) => !isGone(domain))), Effect.catchTag(["RailwayNotFound", "NotFound"], () => Effect.succeed([])));
...
const alreadyExists = (message) => /already exists|already in use|already taken|duplicate/i.test(message);
const waitUntilGone = (customDomainId, projectId) => getById(customDomainId, projectId).pipe(Effect.map((domain) => domain === undefined), Effect.repeat({
