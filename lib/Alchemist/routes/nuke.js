...
import { Progress, withSpanEvents } from "../Progress.js";
import { buildStackProviders, DEFAULT_ENTRYPOINT, } from "../Session.js";
/** Enumerate everything the stack's registered providers can see. */
export const scan = Effect.fn("Alchemist.nuke.scan")(function* (input) {
    const report = withSpanEvents(yield* Progress);
    const debug = yield* Config.string("DEBUG").pipe(Config.withDefault(""), Effect.map((value) => value.length > 0));
    ...
    const { resources, failures } = yield* Nuke.list({
        ...
        onProvider: (provider, count, error) => report({
    ...
    return { mode: input.mode, resources, failures, context };
});
/**
 ...
 */
export const execute = Effect.fn("Alchemist.nuke.execute")(function* (input) {
    const report = withSpanEvents(yield* Progress);
    const keys = new Map(input.resources.map((resource, index) => [resource, `nuke/${index}`]));
    const status = (resource, status, message) => report({
    ...
    return yield* Nuke.destroy({
        ...
        onHeld: (resource, blockedBy) => status(resource, "skipped", `Held back by ${blockedBy.join(", ")}`),
        onDeleted: (resource) => status(resource, "deleted").pipe(Effect.andThen(report({
            _tag: "nuke.resource.deleted",
            provider: resource.providerId,
            resource: resource.displayName,
        }))),
        onFailed: (resource, message) => status(resource, "fail", message).pipe(Effect.andThen(report({
